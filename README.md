...
- 日本語の計算条件入力から resolver -> adapter -> formatter を通して、ダメージ結果を表示できる
- 計算条件を `schemaVersion` 付き JSON として copy / import し、代表条件を round-trip できる
- GitHub Actions で validation / test / build / GitHub Pages deploy を実行できる
...
GitHub 側の Pages 設定は、Repository settings -> Pages -> Source を `GitHub Actions` にする。
...
UI は generated options と manual overlay を使い、resolver 候補、計算条件、formatter result を確認できる。
未解決や曖昧一致は resolver result として表示し、adapter へは解決済み canonical name だけを渡す。
...
`src/App.tsx` は、日本語入力から canonical name 解決、`@smogon/calc` adapter、formatter result までをブラウザで確認する最小 UI。
上部の `resolver input` で Pokemon / Move / Item / Ability / Nature / Type の候補を確認し、中央の一覧と右側の `resolver trace` で一致理由を見られる。
`計算条件` では攻撃側、防御側、技、持ち物、特性、性格、天候、フィールド、ひかりのかべを入力し、解決済み canonical name だけを adapter へ渡して計算する。
下部の `formatter result` は、その計算結果を `src/formatters/jaResultFormatter.ts` で日本語表示したもの。
...
日本語名、別名、表示名の補正は generated JSON を手で直さず、`src/data/overrides/ja-aliases.json` と `src/data/overrides/ja-label-overrides.json` に追加する。
...
- Ability の ChampionCreator 新特性 4 件は `needs-confirmation` として扱い、`@smogon/calc` に存在しないものは adapter で計算しない
- Type `unknown -> ???` は calc catalog の empty id と対応するため、`validate:ja-mapping` では warning として残す
- 公式イラスト asset により deploy artifact は約 159MB 増える
